...
class Bucket():
	...
	def borrow(self,other_bucket):
		...
		if other_bucket==self.right_bucket:
			...
			if self.count==0:
				self.insert_item(temp_item.key,temp_item.value)
			else:
				self.end.child=other_bucket.start
			...
			#print("ERROR! borrowing must be from sibling bucket")
			#print(other_bucket==self.left_bucket,other_bucket==self.right_bucket,self==other_bucket.left_bucket,self==other_bucket.right_bucket)
			raise HaltException("ERROR! borrowing must be from sibling bucket")#exit()
	...
	def print_bucket(self):
		temp_item=self.start
		print("printing bucket started")
		#print(self.middle.child)
		while temp_item != None:
			s=''
			if temp_item==self.start:
				s+=' (S) '
			...
			temp_item=temp_item.child
		print("smaller count : ",self.smaller_count,' larger count : ',self.larger_count, " count= : ",self.count)
		print("printing bucket finished")		


	def before_split_merge_check_invariants(self):
		#check that start middle end pointer are right
		#check all items point to current bucket
		return
	def check_invariants(self):
		#check if either start,middle,end is None at one point in time
		#check buckets are sorted
		#check every child points to its parent and vice versa
		return
